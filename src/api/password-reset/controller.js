...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Openmdbapi account.<br>
        ...
        &mdash; Openmdbapi Team
      `
      return sendMail({ toEmail: email, subject: 'Openmdbapi - Password Reset', content })
    })
